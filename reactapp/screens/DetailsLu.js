...
const DetailsLu = ({ route, navigation, props }) => {
  ...
  React.useEffect(() => {
    let { bookLuClicked } = route.params;
    setTitle(bookLuClicked.title);
    setCover(bookLuClicked.cover);
    setAuthor(bookLuClicked.author);
    setBookId(bookLuClicked._id);
    setDescription(bookLuClicked.description);
    console.log(bookLuClicked.cover, "<---- C MON ITEM");
  }, []);
    ...
    return (
      <View style={{ flex: 1, flexDirection: "row" }}>
        ...
        <TouchableOpacity
          ...
          onPress={() => saveBookInfosDonne(title, cover)}
        >
          <Text style={{ ...FONTS.h3, color: COLORS.white }}>Livre Donné </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={{
...
export default connect(mapStateToProps, null)(DetailsLu);
