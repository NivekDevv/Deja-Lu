...
const DetailsAllList = ({ route, navigation, props }) => {
  ...
  React.useEffect(() => {
    let { bookAllClicked } = route.params;
    setTitle(bookAllClicked.title);
    setCover(bookAllClicked.cover);
    setAuthor(bookAllClicked.author);
    setBookId(bookAllClicked._id);
    setDescription(bookAllClicked.description);
    setBookStatus(bookAllClicked.dejalu);
  ...
  var myStatus = () => {
    ...
    if (bookStatus === "false") {
      return <Text style={{ ...FONTS.body4, color: "red" }}>Non</Text>;
    }
    if (bookStatus === "donné") {
      return <Text style={{ ...FONTS.body4, color: COLORS.white }}>Oui</Text>;
    }
...
export default connect(mapStateToProps, null)(DetailsAllList);
